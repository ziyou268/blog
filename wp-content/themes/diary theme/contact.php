...
if(isset($_POST['submitted'])) {
		...
		if(trim($_POST['contactName']) === '') {
			$nameError = __("你忘了输入您的姓名或昵称。", "site5framework");
			$hasError = true;
		} else {
		...
		if(trim($_POST['email']) === '')  {
			$emailError = __("请输入您的邮箱地址", "site5framework");
			$hasError = true;
		} else if (!eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim($_POST['email']))) {
			$emailError = __("您输入了无效的电子邮件地址", "site5framework");
			$hasError = true;
		} else {
		...
		if(trim($_POST['comments']) === '') {
			$commentError = __("您忘了输入评论", "site5framework");
			$hasError = true;
		} else {
				...
				<section class="pageText">
					...
					<?php }?>
						...
						<label for="nameinput"><?php _e("Your name", "site5framework"); ?> *</label>
							...
							<span class="error" <?php if($nameError != '') echo 'style="display:block;"'; ?>><?php _e("您忘了输入您的姓名或昵称。", "site5framework");?></span>
						<label for="emailinput"><?php _e("Your email", "site5framework"); ?>*</label>
							<input type="text" id="emailinput" name="email" value="<?php if(isset($_POST['email']))  echo $_POST['email'];?>" class="requiredField email"/>
							  <span class="error" <?php if($emailError != '') echo 'style="display:block;"'; ?>><?php _e("请输入您的邮箱。", "site5framework");?></span>
						<label for="commentinput"><?php _e("Your message", "site5framework"); ?>*</label>
							<textarea cols="20" rows="7" id="commentinput" name="comments" class="requiredField"><?php if(isset($_POST['comments'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['comments']); } else { echo $_POST['comments']; } } ?></textarea>
							  <span class="error" <?php if($commentError != '') echo 'style="display:block;"'; ?>><?php _e("您忘了输入评论。", "site5framework");?></span>
						<br />
